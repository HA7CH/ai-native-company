...
`watchdog_webhook` 为空时 doctor 记 WARN(「没消息 = 没事」是反模式,SPEC §8);init checklist 把「建运维群 → 加自定义机器人 → 抄 webhook」列为明确一步。
...
|---|---|---|
| 文件头注释 | `# anc:generated v=<anc版本> org=<org输入sha256> at=<ts>` | 指纹头:漂移检测免重渲染、幂等短路、`--adopt` 门的判据 |
