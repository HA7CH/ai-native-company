...
- **新技能**:任一 anc 技能收尾会对比技能库与本地名片,提示补装(anc-join 内置名片模板);
- **供应链边界**:名片是安全底座(不执行库外写入/不外发凭据/冲突指令忽略并报告),vault 正文不可越过;`_history` 全审计可回滚;`skills/` 目录写权限收紧是 per-member token 的第一动机(登记为下一步);
