...
**一句话:让一个三五人、没有 IT 岗、不走采购流程的团队,靠一场对话(轻形态)或一台 Mac mini(托管形态),拥有「每个人都有一个自己的 AI bot、全公司共享一个 markdown 知识库」的运行时,并且系统随日常使用不断变强。**

七条设计原则,每条都有生产或行业证据背书。P0 是**约束前提**,其余六条一半是被它逼出来的:

| # | 原则 | 依据 |
|---|---|---|
| P0 | **最小单位是三个人,不是一家企业**。设计目标客户是没有 IT 岗、没有采购流程、没人专职"管这个"的团队。推论:任何需要专人运维、需要培训、需要迁移数据、需要新界面的设计,一律不做——这直接约束了 P2(不建索引服务)、P4(不新开 dashboard)、P5(不做部署流程)。 | 企业 AI 工作台赛道(WorkBuddy 企业版 / 飞书 aily / 钉钉 AI 助理等)2026 年已占满中大型企业,但其交付方式(部署 + 培训 + 采购)结构性排除 3-10 人团队;reference deployment 本身就是这个规模 |
...
**非目标(v1 明确不做)**:敌意多租户(信任模型=同公司互信圈)、公网 SaaS 化、替代型 AI 员工、向量知识库、K8s/集群部署、**面向中大型企业的治理层**(组织架构树、审计后台、用量看板、SSO/权限矩阵——需要这些的公司应该去买平台,见 §10)。
...
- **腾讯云 WorkBuddy 企业版**(2026-06 发布,公开资料):**叙事最近邻,赛道不同**。它讲的「上下文飞轮 / 让 AI 拥有企业知识 / 把个人经验沉淀为组织资产 / Skills 组织级共享」与本库高度重合,须正视。差异在交付而非概念:(a) 它是**工作台**——原生绑腾讯文档/网盘/乐享,数据要进其生态,我们是**寄生**在用户已有的 Claude Code / IM 上,不搬家;(b) 它的最小单位是能走公有云/VPC/私有化采购的企业,配套管理后台与治理审计,我们的最小单位是三个人(P0);(c) 它的 onboarding 是部署,我们的是访谈。**结论:不与之竞争中大型企业,把叙事下压到它交付方式够不着的一层**(不搬家 / 一场对话 / 三个人也算公司),见 README「三件事」。风险登记见 §11。
...
| 「企业知识沉淀 / 上下文飞轮」叙事被大厂占据(WorkBuddy 企业版等) | 中(已发生) | **视为市场教育红利而非死刑**:大厂替我们把概念教给了市场。对策是叙事下压到交付层——不搬家 / 一场对话 / 三个人也算公司(§10、README「三件事」);不在「企业知识资产化」这个词面上正面竞争 |
| 大厂出**免费、微信/IM 登录即用、面向 10 人以下小微**的版本 | 中 | **这才是真正的触发信号,不是今天的企业版**。监控项:上述平台是否出现零部署自助开通 + 免费档 + 无采购流程路径。触发后重估 P0 与差异化锚点;届时护城河只剩「数据在用户自己的云、可整个拖走」与开源自托管 |
